...
public class UserController {
    ...
    public String login(@RequestParam("username") String username,
        ...
        if(user == null) {
            model.addAttribute("errMsg", "해당 id의 사용자가 없어욘,,");
            return context + "/login-failed";
        }

        if(!userService.isPasswordValid(user, password)) {
            model.addAttribute("errMsg", "비밀번호가 틀려잇~~~~~~!!~~!~!~");
            return context + "/login-failed";
        }
    ...
    public String register(
        ...
        if (username.isEmpty() || password.isEmpty()) {
            model.addAttribute("errMsg", "아이디 또는 비밀번호가 누락되었슴다~!");
            return context + "/register-failed";
        }

        User user = userService.findUserByUsername(username);
        if (user != null) {
            model.addAttribute("errMsg", "동일한 아이디를 가지는 사용자가 존재 합 니 다 ~ ! ! ");
            return context + "/register-failed";
        }
